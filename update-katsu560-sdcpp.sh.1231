...
CMKOPTNOAVX="-DGGML_AVX=OFF -DGGML_AVX2=OFF -DGGML_AVX512=OFF -DGGML_AVX512_VBMI=OFF -DGGML_AVX512_VNNI=OFF -DGGML_FMA=OFF -DGGML_F16C=OFF $CMKOPTBLAS $CMKOPTTEST $CMKOPTEX"
CMKOPTAVX="-DGGML_AVX=ON -DGGML_AVX2=OFF -DGGML_AVX512=OFF -DGGML_AVX512_VBMI=OFF -DGGML_AVX512_VNNI=OFF -DGGML_FMA=OFF -DGGML_F16C=ON $CMKOPTBLAS $CMKOPTTEST $CMKOPTEX"
CMKOPTAVX2="-DGGML_AVX=ON -DGGML_AVX2=ON -DGGML_AVX512=OFF -DGGML_AVX512_VBMI=OFF -DGGML_AVX512_VNNI=OFF -DGGML_FMA=ON -DGGML_F16C=ON $CMKOPTBLAS $CMKOPTTEST $CMKOPTEX"
CMKOPTNONE="$CMKOPTBLAS $CMKOPTTEST $CMKOPTEX"
CMKOPT="$CMKOPTNONE"
CMKOPT2=""
...
usage()
{
	...
	echo "  cmd ... 15st|5safetensors  bra7  yyi25|yayoi25"
...
# func:cp_mk_script ver: 2023.12.29
...
do_mk_script()
{
	...
	DFIXSH=`get_datefile_date md $DFFIXSH`
...
# func:do_sd ver: 2023.12.29
# generate image by sd
# do_sd FHEAD VARPROMPT VARNGPROMPT MODEL TYPE QTYPE
do_sd()
{
	local FHEAD VARPROMPT PROMPTTXT VARNGPROMPT NGPROMPTTXT MODEL TYPE QTYPE RETCODE DT SEED

	if [ x"$1" = x ]; then
		msg "${ESCERR}do_sd: need filename head FHEAD, skip${ESCBACK}"
		return $ERR_BADARG
	fi
	FHEAD="$1"

	if [ x"$2" = x ]; then
		msg "${ESCERR}do_sd: need VARPROMPT, skip${ESCBACK}"
		return $ERR_BADARG
	fi
	VARPROMT="$2"
	...
	if [ x"$3" = x ]; then
		msg "${ESCERR}do_sd: need VARNGPROMPT, skip${ESCBACK}"
		return $ERR_BADARG
	fi
	VARNNGPROMT="$3"
	...
	if [ x"$4" = x ]; then
		msg "${ESCERR}do_sd: need MODEL, skip${ESCBACK}"
		return $ERR_BADARG
	fi
	MODEL="$4"
	...
	if [ x"$5" = x ]; then
		if [ x"$TYPE" = x ]; then
			msg "${ESCERR}do_sd: need TYPE, skip${ESCBACK}"
			return $ERR_BADARG
		fi
	else
		TYPE="$5"
...
do_sdv14()
{
	...
	if [ ! x"$MAOPT" = xNOMAKE ]; then
		...
		msg "make $ALLBINS"
		make $ALLBINS || die 281 "make $ALLBINS failed"
...
do_14ckpt()
{
	...
	if [ ! x"$MAOPT" = xNOMAKE ]; then
		if [ $MKCLEAN -eq $RET_FALSE -a $NOCLEAN -eq $RET_FALSE ]; then
			msg "make clean"
			make clean || die 290 "make clean failed"
			MKCLEAN=$RET_TRUE
		fi
		msg "make $ALLBINS"
		make $ALLBINS || die 291 "make $ALLBINS failed"
		BINS=""; for i in $ALLBINS ;do BINS="$BINS bin/$i" ;done
		msg "cp -p $BINS $DIRNAME/"
		cp -p $BINS $DIRNAME || die 292 "can't cp $BINS"
	fi
...
do_2ckpt()
{
	...
	if [ ! x"$MAOPT" = xNOMAKE ]; then
		if [ $MKCLEAN -eq $RET_FALSE -a $NOCLEAN -eq $RET_FALSE ]; then
			msg "make clean"
			make clean || die 300 "make clean failed"
			MKCLEAN=$RET_TRUE
		fi
		msg "make $ALLBINS"
		make $ALLBINS || die 301 "make $ALLBINS failed"
		BINS=""; for i in $ALLBINS ;do BINS="$BINS bin/$i" ;done
		msg "cp -p $BINS $DIRNAME/"
		cp -p $BINS $DIRNAME || die 302 "can't cp $BINS"
	fi
...
do_2st()
{
	...
	if [ ! x"$MAOPT" = xNOMAKE ]; then
		if [ $MKCLEAN -eq $RET_FALSE -a $NOCLEAN -eq $RET_FALSE ]; then
			msg "make clean"
			make clean || die 310 "make clean failed"
			MKCLEAN=$RET_TRUE
		fi
		msg "make $ALLBINS"
		make $ALLBINS || die 311 "make $ALLBINS failed"
		BINS=""; for i in $ALLBINS ;do BINS="$BINS bin/$i" ;done
		msg "cp -p $BINS $DIRNAME/"
		cp -p $BINS $DIRNAME || die 312 "can't cp $BINS"
	fi
...
do_sdnano()
{
	...
	if [ ! x"$MAOPT" = xNOMAKE ]; then
		if [ $MKCLEAN -eq $RET_FALSE -a $NOCLEAN -eq $RET_FALSE ]; then
			msg "make clean"
			make clean || die 320 "make clean failed"
			MKCLEAN=$RET_TRUE
		fi
		msg "make $ALLBINS"
		make $ALLBINS || die 321 "make $ALLBINS failed"
		BINS=""; for i in $ALLBINS ;do BINS="$BINS bin/$i" ;done
		msg "cp -p $BINS $DIRNAME/"
		cp -p $BINS $DIRNAME || die 322 "can't cp $BINS"
	fi
...
do_21st()
{
	...
	if [ ! x"$MAOPT" = xNOMAKE ]; then
		if [ $MKCLEAN -eq $RET_FALSE -a $NOCLEAN -eq $RET_FALSE ]; then
			msg "make clean"
			make clean || die 330 "make clean failed"
			MKCLEAN=$RET_TRUE
		fi
		msg "make $ALLBINS"
		make $ALLBINS || die 331 "make $ALLBINS failed"
		BINS=""; for i in $ALLBINS ;do BINS="$BINS bin/$i" ;done
		msg "cp -p $BINS $DIRNAME/"
		cp -p $BINS $DIRNAME || die 332 "can't cp $BINS"
	fi
...
do_15st()
{
	...
	if [ ! x"$MAOPT" = xNOMAKE ]; then
		if [ $MKCLEAN -eq $RET_FALSE -a $NOCLEAN -eq $RET_FALSE ]; then
			msg "make clean"
			make clean || die 340 "make clean failed"
			MKCLEAN=$RET_TRUE
		fi
		msg "make $ALLBINS"
		make $ALLBINS || die 341 "make $ALLBINS failed"
		BINS=""; for i in $ALLBINS ;do BINS="$BINS bin/$i" ;done
		msg "cp -p $BINS $DIRNAME/"
		cp -p $BINS $DIRNAME || die 342 "can't cp $BINS"
	fi
...
do_sdtb()
{
	...
	if [ ! x"$MAOPT" = xNOMAKE ]; then
		if [ $MKCLEAN -eq $RET_FALSE -a $NOCLEAN -eq $RET_FALSE ]; then
			msg "make clean"
			make clean || die 350 "make clean failed"
			MKCLEAN=$RET_TRUE
		fi
		msg "make $ALLBINS"
		make $ALLBINS || die 351 "make $ALLBINS failed"
		BINS=""; for i in $ALLBINS ;do BINS="$BINS bin/$i" ;done
		msg "cp -p $BINS $DIRNAME/"
		cp -p $BINS $DIRNAME || die 352 "can't cp $BINS"
	fi
...
do_sdxltb()
{
	...
	if [ ! x"$MAOPT" = xNOMAKE ]; then
		if [ $MKCLEAN -eq $RET_FALSE -a $NOCLEAN -eq $RET_FALSE ]; then
			msg "make clean"
			make clean || die 360 "make clean failed"
			MKCLEAN=$RET_TRUE
		fi
		msg "make $ALLBINS"
		make $ALLBINS || die 361 "make $ALLBINS failed"
		BINS=""; for i in $ALLBINS ;do BINS="$BINS bin/$i" ;done
		msg "cp -p $BINS $DIRNAME/"
		cp -p $BINS $DIRNAME || die 362 "can't cp $BINS"
	fi
...
do_main()
{
	...
	if [ ! x"$MAOPT" = xNOMAKE ]; then
		...
		msg "make $ALLBINS"
		make $ALLBINS || die 262 "make $ALLBINS failed"
		BINS=""; for i in $ALLBINS ;do BINS="$BINS bin/$i" ;done
		msg "cp -p $BINS $DIRNAME/"
		cp -p $BINS $DIRNAME || die 263 "can't cp $BINS"
	fi
...
#echo "  cmd ... sd14|sd14only sdnano|sdnanoonly 14ckpt 2ckpt 2safetensors/sd2st"
case $CMD in
